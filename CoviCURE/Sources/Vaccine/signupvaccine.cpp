    ...
    ui(new Ui::Signupvaccine)
{
    ...
    ui->combo_vaccine_name->addItem("AstraZeneca");
     ui->combo_vaccine_name->addItem("Moderna");
      ui->combo_vaccine_name->addItem("Johnson & Johnson");
       ui->combo_vaccine_name->addItem("Pfizer");
        ui->combo_vaccine_name->addItem("Vero Cell");
         ui->combo_vaccine_name->addItem("Sputnik V");
}
    ...
    Register.connectDB("user");
     ...
     if(!Register.connectionDB()) {
         ...
         return ;
          ...
          QString vaccineName = ui->combo_vaccine_name->currentText();
          QString nextDueDate = ui->duedate->text();
         // QString dose1 = QString::xx;

          QSqlQuery qry;

          qry.prepare("INSERT INTO vaccine (name, sex, dob, date, mobile, address, license, state, nation, pin, vaccineName, nextDueDate)"
                      "VALUES (:name, :sex, :dob, :date, :mobile, :address, :license, :state, :nation, :pin, :vaccineName, :nextDueDate)");
          ...
          qry.bindValue(":vaccineName", vaccineName);
          qry.bindValue(":nextDueDate", nextDueDate);

          if(qry.exec()){
               QMessageBox:: information(this, "Registered", "You have been registered as a user. Your PIN number is '"+pin+"'");
          }
          else{
